...
class PLMSSampler(object):
    ...
    def sample(self,
        ...
        samples, intermediates, x_T_copy, pred_x0 = self.plms_sampling(conditioning, size,
                                                    callback=callback,
    ...
    def sample_student(self,
        ...
        samples, intermediates, x_T_copy, a_t, pred_x0 = self.plms_sampling_student(conditioning, size,
                                                    callback=callback,
                                                    img_callback=img_callback,
                                                    quantize_denoised=quantize_x0,
                                                    mask=mask, x0=x0,
    ...
    @torch.enable_grad()
    def plms_sampling_student(self, cond, shape,
                      x_T=None, ddim_use_original_steps=False,
                      callback=None, timesteps=None, quantize_denoised=False,
                      mask=None, x0=None, img_callback=None, log_every_t=100,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
        ...
        for i, step in enumerate(iterator):
            ...
            outs = self.p_sample_plms_student(img, cond, ts, index=index, use_original_steps=ddim_use_original_steps,
                                      quantize_denoised=quantize_denoised, temperature=temperature,
                                      noise_dropout=noise_dropout, score_corrector=score_corrector,
                                      corrector_kwargs=corrector_kwargs,
                                      unconditional_guidance_scale=unconditional_guidance_scale,
    ...
    @torch.enable_grad()
    def p_sample_plms_student(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., unconditional_conditioning=None, old_eps=None, t_next=None):
        b, *_, device = *x.shape, x.device

        def get_model_output_student(x, t):
            if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
                e_t = self.model.apply_model(x, t, c)
            else:
                x_in = torch.cat([x] * 2)
        ...
        def get_x_prev_and_pred_x0_student(e_t, index):
            # select parameters corresponding to the currently considered timestep
            a_t = torch.full((b, 1, 1, 1), alphas[index], device=device)
            a_prev = torch.full((b, 1, 1, 1), alphas_prev[index], device=device)
            sigma_t = torch.full((b, 1, 1, 1), sigmas[index], device=device)
        ...
        e_t = get_model_output_student(x, t)
        if len(old_eps) == 0:
            # Pseudo Improved Euler (2nd order)
            x_prev, pred_x0, a_t = get_x_prev_and_pred_x0_student(e_t, index)
            e_t_next = get_model_output_student(x_prev, t_next)
            e_t_prime = (e_t + e_t_next) / 2
        elif len(old_eps) == 1:
            # 2nd order Pseudo Linear Multistep (Adams-Bashforth)
            e_t_prime = (3 * e_t - old_eps[-1]) / 2
        ...
        x_prev, pred_x0, a_t = get_x_prev_and_pred_x0_student(e_t_prime, index)

        return x_prev, pred_x0, e_t, a_t
